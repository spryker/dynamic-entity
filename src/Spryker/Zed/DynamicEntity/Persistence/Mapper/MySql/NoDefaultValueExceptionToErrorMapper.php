...
class NoDefaultValueExceptionToErrorMapper implements DatabaseExceptionToErrorMapperInterface
{
    /**
     * @var string
     */
    protected const ERROR_CODE_GENERAL_ERROR = 'HY000';
     ...
     * @var string
     */
    protected const NO_DEFAULT_VALUE_REGEX = '/Field \'(.*?)\' doesn\'t have a default value/';
    ...
    public function isApplicable(Exception $exception): bool
    {
        ...
        $errorMatches = (bool)preg_match(static::NO_DEFAULT_VALUE_REGEX, $previousException->getMessage());

        return $code === static::ERROR_CODE_GENERAL_ERROR && $errorMatches === true;
    }
    ...
    public function mapExceptionToErrorMessage(Exception $exception): ?string
    {
        ...
        if (preg_match(static::NO_DEFAULT_VALUE_REGEX, $previousException->getMessage(), $matches)) {
            return $matches[1];
        }
